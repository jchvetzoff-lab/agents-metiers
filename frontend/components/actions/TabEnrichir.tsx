    ...
    for (const code of selectedCodes) {
      ...
      try {
        ...
        setResults(prev => [{
          ...
          nom,
          type: "error", 
          message: err.message || "Erreur lors de l'enrichissement"
        }, ...prev]);
      }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-indigo-500/10 to-purple-500/10 border border-indigo-500/20 rounded-xl p-5">
        ...
        <p className="text-sm text-gray-400">
          Sélectionnez les fiches brouillon à enrichir. L'IA génère automatiquement : description détaillée, compétences,
          salaires, perspectives, conditions de travail, mobilité, etc.
        </p>
      </div>

      {/* Progress bar */}
      {processing && (
        <div className="bg-[#0c0c1a] rounded-xl border border-indigo-500/20 p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-indigo-400">
              Enrichissement en cours... ({processedCount}/{totalCount})
            </span>
      ...
      <div className="bg-[#0c0c1a] rounded-2xl border border-white/[0.06] overflow-hidden">
              ...
              {fiches.length > 0 && (
                <>
                  ...
                  <button
                    ...
                    className="px-6 py-2 bg-indigo-600 text-white rounded-xl text-sm font-medium hover:bg-indigo-700 transition disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                    {processing ? `En cours... (${processedCount}/${totalCount})` : `Enrichir les ${selected.size} fiches sélectionnées`}
                  </button>
                </>
              )}
            </div>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2">
              <button
                ...
                className="px-3 py-1 text-sm border border-white/[0.1] rounded-lg disabled:opacity-40 hover:border-indigo-500/30 transition"
              >
                Précédent
              </button>
              <span className="text-sm text-gray-400">
        ...
        <div className="divide-y divide-white/[0.06] max-h-[500px] overflow-y-auto">
          {loading ? (
            ...
            <EmptyState 
              message={search ? `Aucune fiche brouillon pour "${search}"` : "Aucune fiche brouillon à enrichir"} 
            />
          ) : (
            filteredFiches.map((fiche) => (
              ...
              >
                <input
                  ...
                  disabled={processing}
                  className="w-4 h-4 rounded border-white/[0.1] text-indigo-400 focus:ring-indigo-500 cursor-pointer disabled:opacity-40"
                />
