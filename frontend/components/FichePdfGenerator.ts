import { FicheDetail, VarianteDetail } from "@/lib/api";
...
export async function generatePdf({ fiche, appliedVariante, filterGenre }: PdfGeneratorParams) {
  const av = appliedVariante;
  const lang = av?.langue || "fr";
  ...
  if (showStats) {
    ...
    if (d.perspectives?.tendance) {
      ...
      // Evolution card
      if (d.perspectives.evolution_5ans || av?.evolution_5ans) {
        const evolution = av?.evolution_5ans || d.perspectives.evolution_5ans!;
        fill(C.gray50);
        stroke(C.gray200);
        ...
        pdf.setFont("helvetica", "normal");
        txt(C.gray700);
        const evoLines = pdf.splitTextToSize(evolution, halfW - 16);
        let ey = y + 13;
