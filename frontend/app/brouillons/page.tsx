...
export default function BrouillonsPage() {
  ...
  const loadFiches = useCallback(async () => {
    try {
      ...
      // Sort by score ascending (most needing attention first)
  ...
  }, [debouncedSearch, debouncedCompetences, searchMode, page]);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          <div className="flex items-center justify-center gap-4 mb-4">
            ...
            <h1 className="text-3xl md:text-4xl font-serif font-bold gradient-text">Brouillons</h1>
          </div>
          <p className="text-lg text-text-muted">
            Fiches en cours de rédaction — triées par priorité d&apos;enrichissement
          </p>
        </div>
        ...
        <div className="sojai-card mb-8">
          ...
          {searchMode === "fuzzy" ? (
            <div>
              ...
              />
              <p className="text-xs text-gray-400 mt-2">Trouve les métiers meme avec des fautes de frappe ou des noms approchants</p>
            </div>
          ) : (
            <div>
              ...
              />
              <p className="text-xs text-gray-400 mt-2">Trouve les métiers qui requierent cette compétence ou formation</p>
            </div>
          )}

          <div className="mt-4 flex items-center justify-between text-sm text-text-muted">
            <span>{total} fiche{total > 1 ? "s" : ""} brouillon{total > 1 ? "s" : ""}</span>
            {(search || searchCompetences) && (
              <button onClick={() => { setSearch(""); setSearchCompetences(""); setPage(0); }} className="text-primary-purple hover:underline">
                Réinitialiser
              </button>
            )}
          </div>
                    ...
                    {fiches.length === 0 && (
                      <tr>
                        <td colSpan={5} className="p-8 text-center text-text-muted">
                          Aucune fiche brouillon trouvée{search ? ` pour "${search}"` : ""}
                        </td>
                      </tr>
                    )}
                    {fiches.map((fiche) => {
